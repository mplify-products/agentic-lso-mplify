...
**Revealing text on the next press: `.reveal`.** Any element with `.reveal` starts hidden and is shown, one per press in DOM order, before the slide's diagram stops (if any) begin. A slide with R reveals and N stepped diagrams has R+2N+1 stops. Going back hides them again in reverse. Print shows every `.reveal`. Use it for a question-then-answer beat, like the "Pace of change" slide in the Boston Thursday deck.
